...
def make_layer(block, num_blocks, attention_mechanism, num_landmarks, iters, **kwarg):
    ...
    for _ in range(num_blocks):
        layers.append(block(attention_mechanism, num_landmarks, iters, **kwarg))
...
class PnPNystraAttention(nn.Module):
    ...
    def __init__(self, num_landmarks, iters, dim, window_size, num_heads, qkv_bias=False, qk_scale=None):
        super().__init__()
        self.window_size = window_size
        self.num_heads = num_heads
        self.dim = dim
    ...
    def forward(self, x):
        ...
        qkv = self.qkv_self(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # B_, nH, N, C
        x_out = self.attention(q * self.scale, k, v, (B_, N, C))
    ...
    def moore_penrose_iter_pinv(self, x, iters = 3):
        device = x.device
    ...
    def attention(self, q, k, v, x_shape):
        ...
        hi, wi = closest_square_factors(N)

        q_m = F.adaptive_avg_pool2d(q.reshape(B_*self.num_heads, hi, wi, self.dim//self.num_heads).permute(0, 3, 1, 2), output_size = (h, w)).permute(0, 2, 3, 1).reshape(B_, self.num_heads, num_landmarks, self.dim//self.num_heads)
        k_m = F.adaptive_avg_pool2d(k.reshape(B_*self.num_heads, hi, wi, self.dim//self.num_heads).permute(0, 3, 1, 2), output_size = (h, w)).permute(0, 2, 3, 1).reshape(B_, self.num_heads, num_landmarks, self.dim//self.num_heads)
...
class STL(nn.Module):
    ...
    def __init__(self,
        ...
        elif attention_mechanism == 'pnp_nystra':
            self.attn = PnPNystraAttention(num_landmarks, iters, dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias,
                                                                qk_scale=qk_scale)
...
class STG(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            STL(attention_mechanism = attention_mechanism,
...
class RSTB(nn.Module):
    ...
    def __init__(self, attention_mechanism, num_landmarks, iters, **kwargs):
        super(RSTB, self).__init__()
        self.input_resolution = kwargs['input_resolution']

        self.residual_group = STG(attention_mechanism, num_landmarks, iters,**kwargs)
...
class RSTBWithInputConv(nn.Module):
    ...
    def __init__(self, attention_mechanism, num_landmarks, iters, in_channels=3, kernel_size=(1, 3, 3), stride=1, groups=1, num_blocks=2, **kwargs):
        super().__init__()
        ...
        main.append(make_layer(RSTB, num_blocks, attention_mechanism, num_landmarks, iters, **kwargs))
...
class RVRT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.upscale == 4:
            # video sr
            self.feat_extract = RSTBWithInputConv(attention_mechanism, num_landmarks, iters, in_channels=3,
                                                  kernel_size=(1, 3, 3),
            ...
            self.feat_extract = nn.Sequential(Rearrange('n d c h w -> n c d h w'),
                                              ...
                                              RSTBWithInputConv(attention_mechanism,
                                                                num_landmarks,
                                                                iters,
        ...
        for i, module in enumerate(modules):
            ...
            # feature propagation
            self.backbone[module] = RSTBWithInputConv(attention_mechanism=attention_mechanism, num_landmarks=num_landmarks, 
                                                     iters = iters,
        ...
        # reconstruction
        self.reconstruction = RSTBWithInputConv(attention_mechanism=attention_mechanism, num_landmarks=num_landmarks, 
                                               iters = iters,
