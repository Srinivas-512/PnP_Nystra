...
def main():
    ...
    if args.mech == 'original':
        for window_size in [64]:
            model = prepare_model_dataset(args, window_size, 16, 1)
            model.eval()
    ...
    if args.mech == 'pnp_nystra':
        for window_size in [64]:
            for num_landmarks in [32]:
                for iters in [6]:
                    model = prepare_model_dataset(args, window_size, num_landmarks, iters)
                    model.eval()
                    model = model.to(device)
...
def prepare_model_dataset(args, window_size, num_landmarks, iters):
    ...
    datasets = ['REDS4']
    args.scale = 4
    args.window_size = [2,window_size,window_size]
    args.nonblind_denoising = False

    pretrained_model = torch.load(args.model_path)
    state_dict = pretrained_model['params'] if 'params' in pretrained_model.keys() else pretrained_model
